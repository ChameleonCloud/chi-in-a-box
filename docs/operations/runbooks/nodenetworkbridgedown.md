...
**Network is misconfigured**: it is likely that, if this is a major/primary network bridge, then all connectivity is down (though a [`NodeExporterDown`](https://github.com/ChameleonCloud/chi-in-a-box/blob/stable/xena/docs/runbooks/\[Runbook]-NodeExporterDown/README.md) alert would be expected in that case, as the monitoring infrastructure likely wouldn't be able to get this information from the problem host at all!).
